...
def install(child_user: str | None = None) -> list[str]:
    ...
    problems: list[str] = []
    if not is_admin():
        return ["Administrator rights are required to install."]
    ...
    if not ok:
        problems.append(f"{AGENT_TASK}: {err}")
    ok, err = create_system_task(AGENT_WATCHDOG_TASK, AGENT_SCRIPT, ["/SC", "MINUTE", "/MO", "5"])
    if not ok:
        problems.append(f"{AGENT_WATCHDOG_TASK}: {err}")

    if user:
        ok, err = create_user_task(OVERLAY_TASK, OVERLAY_SCRIPT, user, ["/SC", "ONLOGON"])
        if not ok:
            problems.append(f"{OVERLAY_TASK}: {err}")
        ok, err = create_user_task(OVERLAY_WATCHDOG_TASK, OVERLAY_SCRIPT, user,
                                   ["/SC", "MINUTE", "/MO", "5"])
        if not ok:
            problems.append(f"{OVERLAY_WATCHDOG_TASK}: {err}")
    else:
        problems.append("No child Windows account set - the lock screen was not installed.")

    failed_policies = browser_policy.apply()
    if failed_policies:
        problems.append(f"Could not write browser DNS policy: {', '.join(failed_policies)}")
